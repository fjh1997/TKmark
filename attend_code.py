...
filtered_ids = [
    ...
    if record["className"] == "信安23-02" and record["courseName"] == "信息安全与代码审计"
]
...
def course(id):
    response = requests.get(
        f'https://newca.zjtongji.edu.cn/attendng/attend/m/teaRecord/getById/{id}?_t=1736737564790',
        cookies=cookies,
        headers=headers,
    )
    data=response.json()
    ...
    for stu in data["result"]['2']:
        print(stu['stuName']+'|'+stu['leaveReason'])
    for stu in data["result"]['1']:
        print(stu['stuName']+'|缺勤')
